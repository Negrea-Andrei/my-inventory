...
async function locationCreate(index, name, address) {
  console.log(`Debug: Creating location ${name} with address ${address}`);
  try {
...
async function createLocations() {
  ...
  await Promise.all([
    locationCreate(0, 'Warehouse A', '123 Main St'),
    locationCreate(1, 'Store B', '456 Oak Ave'),
    locationCreate(2, 'Distribution Center C', '789 Maple Ln'),
  ]);
}
